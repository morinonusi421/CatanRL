...
class hexTile():
    ...
    def __str__(self):
        ...
        return (
            f"hex: {self.hex} ({type(self.hex)}),\n"
            f"resource: {self.resource} ({type(self.resource)}),\n"
            f"coord: {self.coord} ({type(self.coord)}),\n"
            f"pixelCenter: {self.pixelCenter} ({type(self.pixelCenter)}),\n"
            f"index: {self.index} ({type(self.index)}),\n"
            f"robber: {self.robber} ({type(self.robber)}),\n"
            f"neighborList: {self.neighborList} ({type(self.neighborList)})"
        )
...
class Vertex():
    ...
    def __str__(self):
        ...
        #return(
        #    f"pixelCoordinates: {self.pixelCoordinates} ({type(self.pixelCoordinates)}),\n"
        #    f"vertexIndex: {self.vertexIndex} ({type(self.vertexIndex)}),\n"
        #)

        return (
            f"vertexIndex: {self.vertexIndex} ({type(self.vertexIndex)}),\n"
            f"pixelCoordinates: {self.pixelCoordinates} ({type(self.pixelCoordinates)}),\n"
            f"edgeList: {self.edgeList} ({type(self.edgeList)}),\n"
            f"adjacentHexList: {self.adjacentHexList} ({type(self.adjacentHexList)}),\n"
            f"edgeState: {self.edgeState} ({type(self.edgeState)}),\n"
            f"state: {self.state} ({type(self.state)}),\n"
            f"port: {self.port} ({type(self.port)}),\n"
            f"isColonised: {self.isColonised} ({type(self.isColonised)})"
        )
